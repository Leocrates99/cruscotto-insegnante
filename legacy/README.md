> ## 🗄️ ARCHIVIATO
> Questa è la **parte Notion** del progetto, **non più attiva**. Il prodotto è ora la SPA
> local-first in [`../web/`](../web/) (vedi il README principale alla radice del repo).
> Qui sotto resta la documentazione originale del costruttore Notion.
>
> **Stato:** congelato (giugno 2026). La CI Notion (`workflows/automations.yml`, `notion.yml`)
> è stata spostata qui e quindi **disattivata**. Il *modello condiviso* (tipi, schema, dati di
> esempio) NON è qui: vive in [`../src/`](../src/) e `../config/`, perché lo usa la SPA.
> **Per riattivarlo:** riporta `package.json`/`tsconfig.json` alla radice (o lavora dentro
> `legacy/`) e aggiusta i percorsi d'import verso il modello in `../src`/`../config`; la
> cronologia git conserva l'ultimo stato funzionante. I percorsi relativi citati più sotto si
> riferiscono alla **radice del repo**.
